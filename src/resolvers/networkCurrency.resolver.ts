...
import { TransactionGroup, TransactionHttp, UInt64 } from 'bitxor-sdk';
    ...
    async resolve(options: CreateProfileOptions): Promise<NetworkCurrency> {
        try {
            ...
            throw new ExpectedError(
                'The CLI cannot get the network currency token description. Pass the network currency token options with the options `namespace-id` and `divisibility`. E.g.: --namespace-id BitxorBXR --divisibility 6',
            );
        }
    }
}
