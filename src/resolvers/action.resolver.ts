...
import { ActionValidator, LinkActionValidator, TokenSupplyChangeActionValidator } from '../validators/action.validator';
...
export class SupplyActionResolver implements Resolver {
     ...
     */
    async resolve(options: Options, altText?: string, altKey?: string): Promise<number> {
        const choices = Object.keys(TokenSupplyChangeAction)
            .filter((key) => Number.isNaN(parseFloat(key)))
            .map((string) => ({
                title: string,
                value: TokenSupplyChangeAction[string as any],
            }));
        ...
        const value = +(await OptionsChoiceResolver(
            ...
            choices,
            'select',
            new TokenSupplyChangeActionValidator(),
        ));
        return value;
    }
}
