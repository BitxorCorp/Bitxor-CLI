...
export class CreateProfileOptions extends ProfileOptions {
    ...
    @option({
        flag: 'u',
        description: '(Optional) When saving profile, provide a Bitxor Node URL. Example: http://localhost:3000',
    })
    url: string;
    ...
    @option({
        flag: 'i',
        description: '(Optional) Namespace Name of the network token. (eg.: BitxorBXR) Required to create the profile offline.',
    })
    namespaceId: string;

    @option({
        flag: 'v',
        description: '(Optional) Divisibility of the network token. (eg.: 6) Required to create the profile offline.',
    })
    divisibility: number;
