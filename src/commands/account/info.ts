...
import { AccountInfo, Address, TokenAmountView, TokenService, MultisigAccountInfo } from 'bitxor-sdk';
...
export class BalanceInfoTable {
    ...
    constructor(public readonly tokensInfo: TokenAmountView[]) {
        if (tokensInfo.length > 0) {
            this.table = new Table({
                style: { head: ['cyan'] },
                head: ['Token Id', 'Relative Amount', 'Absolute Amount', 'Expiration Height'],
            }) as HorizontalTable;
            tokensInfo.map((token: TokenAmountView) => {
                this.table.push([
                    token.fullName(),
                    token.relativeAmount().toLocaleString(),
                    token.amount.toString(),
                    token.tokenInfo.duration.compact() === 0
                        ? 'Never'
                        : token.tokenInfo.startHeight.add(token.tokenInfo.duration).toString(),
                ]);
            });
        }
    }
...
export default class extends ProfileCommand {
    ...
    async execute(options: CommandOptions) {
        ...
        forkJoin([
            ...
            tokenService.tokensAmountViewFromAddress(address).pipe(
                mergeMap((_) => _),
                toArray(),
            ),
