...
describe('Supply action resolver', () => {
    it('default case', async () => {
        ...
        expect(await new SupplyActionResolver().resolve(options)).to.be.equal(TokenSupplyChangeAction.Decrease);
    });

    it('should change key', async () => {
        const options = { key: 'Decrease' } as any;
        expect(await new SupplyActionResolver().resolve(options, 'altText', 'key')).to.be.equal(TokenSupplyChangeAction.Decrease);
    });
});
