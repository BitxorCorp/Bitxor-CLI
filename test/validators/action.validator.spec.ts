...
import { ActionValidator, LinkActionValidator, TokenSupplyChangeActionValidator } from '../../src/validators/action.validator';
...
describe('Token supply change action validator', () => {
    it('default case', () => {
        const value = 'Increase';
        expect(new TokenSupplyChangeActionValidator().validate(value)).to.be.equal(true);
    });

    it('should throw error if action is unknown', () => {
        const value = 'TEST';
        expect(typeof new TokenSupplyChangeActionValidator().validate(value)).to.be.equal('string');
    });
});
